...
error_log_path = ["error_log", "error_log.txt", "errorlog.sql", "error_log.sql", "errorlog.json", "error_log.json", "error_map", "dump.sql", "database_dump.sql", "db.sql", "data_dump.sql", "sql_dump.sql"]
...
def detect_error_log(url: str):
    for error_log_paths in error_log_path:
        ...
        if error_log_request.status_code == 200:
            if "mysql" in error_log_request.text or "syntax error" in error_log_request.text or "uncaught exception" in error_log_request.text or "fatal error" in error_log_request.text:
